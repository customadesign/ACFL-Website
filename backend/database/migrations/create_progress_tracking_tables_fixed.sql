-- =========================================================
-- PROGRESS TRACKING TABLES (FIXED)
-- =========================================================
...
DROP TABLE IF EXISTS public.progress_metrics CASCADE;
DROP TABLE IF EXISTS public.progress_goals CASCADE;
...
CREATE TABLE public.progress_metrics (
    ...
    session_id UUID NULL, -- Remove foreign key constraint for now since sessions table might not exist
...
CREATE TABLE public.session_progress (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id UUID NOT NULL, -- Remove foreign key constraint for now
...
CREATE TABLE public.wellness_checkins (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id UUID NOT NULL REFERENCES public.clients(id) ON DELETE CASCADE,
    session_id UUID NULL, -- Remove foreign key constraint for now
...
CREATE INDEX IF NOT EXISTS idx_progress_metrics_coach ON public.progress_metrics(recorded_by_coach);
CREATE INDEX IF NOT EXISTS idx_progress_metrics_client ON public.progress_metrics(recorded_by_client);
...
CREATE INDEX IF NOT EXISTS idx_wellness_checkins_session ON public.wellness_checkins(session_id);
...
CREATE INDEX IF NOT EXISTS idx_progress_milestones_session ON public.progress_milestones(session_id);
...
-- Add foreign key constraints for sessions table if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sessions') THEN
        -- Add session foreign key constraints
        ALTER TABLE public.progress_metrics
        ADD CONSTRAINT fk_progress_metrics_session
        FOREIGN KEY (session_id) REFERENCES public.sessions(id) ON DELETE SET NULL;

        ALTER TABLE public.session_progress
        ADD CONSTRAINT fk_session_progress_session
        FOREIGN KEY (session_id) REFERENCES public.sessions(id) ON DELETE CASCADE;

        ALTER TABLE public.wellness_checkins
        ADD CONSTRAINT fk_wellness_checkins_session
        FOREIGN KEY (session_id) REFERENCES public.sessions(id) ON DELETE SET NULL;

        ALTER TABLE public.progress_milestones
        ADD CONSTRAINT fk_progress_milestones_session
        FOREIGN KEY (session_id) REFERENCES public.sessions(id) ON DELETE SET NULL;
    END IF;
END $$;