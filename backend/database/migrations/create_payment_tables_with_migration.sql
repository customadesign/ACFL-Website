-- =========================================================
-- PAYMENT SYSTEM TABLES - WITH MIGRATION FROM EXISTING DATA
-- =========================================================
...
-- 2. Migrate existing hourly rates from coaches table to coach_rates
-- This creates a default 60-minute individual session rate for each coach
INSERT INTO public.coach_rates (coach_id, session_type, duration_minutes, rate_cents, title, description, is_active)
SELECT 
  id as coach_id,
  'individual' as session_type,
  60 as duration_minutes,
  CAST(hourly_rate_usd * 100 AS INT) as rate_cents, -- Convert dollars to cents
  'Standard 1-Hour Session' as title,
  'Individual coaching session' as description,
  true as is_active
FROM public.coaches
WHERE hourly_rate_usd IS NOT NULL 
  AND hourly_rate_usd > 0
  AND NOT EXISTS (
    -- Only insert if coach doesn't already have rates
    SELECT 1 FROM public.coach_rates cr 
    WHERE cr.coach_id = coaches.id 
    AND cr.session_type = 'individual' 
    AND cr.duration_minutes = 60
  );
...
-- 5. Payment logs table for audit trail
...
-- 7. Payment schedules for recurring payments (future)
...
-- =========================================================
-- OPTIONAL: Add legacy rate column to coach_rates for reference
-- =========================================================
-- If you want to keep track of which rates came from the old system
ALTER TABLE public.coach_rates 
ADD COLUMN IF NOT EXISTS legacy_hourly_rate DECIMAL(10,2);

-- Update the migrated rates with the original hourly rate
UPDATE public.coach_rates cr
SET legacy_hourly_rate = c.hourly_rate_usd
FROM public.coaches c
WHERE cr.coach_id = c.id 
  AND cr.title = 'Standard 1-Hour Session'
  AND cr.session_type = 'individual'
  AND cr.duration_minutes = 60;

-- =========================================================
-- VERIFICATION QUERY - Run this to see migrated data
-- =========================================================
-- SELECT 
--   c.first_name || ' ' || c.last_name as coach_name,
--   c.hourly_rate_usd as old_hourly_rate,
--   cr.rate_cents / 100.0 as new_hourly_rate,
--   cr.title,
--   cr.session_type,
--   cr.duration_minutes
-- FROM public.coaches c
-- LEFT JOIN public.coach_rates cr ON c.id = cr.coach_id
-- WHERE c.hourly_rate_usd IS NOT NULL
